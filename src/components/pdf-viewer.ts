...
export class PDFViewerElement extends LitElement {
  ...
  render() {
    return html` <slot name="toolbar" ?hidden=${this.hideToolbar}
      ...
      ></pdf-viewer-display>
      <div id="controls">
        <mwc-fab id="zoom-out" icon="remove" mini @click=${this.zoomOut}>
        </mwc-fab>
        <mwc-fab id="zoom-in" icon="add" mini @click=${this.zoomIn}> </mwc-fab>
      </div>`;
  }
